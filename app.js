...
async function bootstrap(){
  try{
    ...
    renderItems(); renderUsers(); renderDashboard(); buildQRSheets();
...
qs('#dlg-item-ok').addEventListener('click', async(e)=>{
  ...
  try{ await api('addItem', { method:'POST', body }); qs('#dlg-item').close(); state.items = await api('items'); renderItems(); buildQRSheets(); renderDashboard(); }
...
function renderUsers(){
  ...
  state.users.forEach(u=>{
    const tr = document.createElement('tr');
    tr.innerHTML = `<td>${u.name}</td><td>${u.id}</td><td><button data-id="${u.id}" class="btn-user-qr primary">QR</button></td>`;
    tbd.appendChild(tr);
  });
...
qs('#import-items').addEventListener('change', async(e)=>{
  ...
  state.items = await api('items'); renderItems(); buildQRSheets(); renderDashboard();
  alert('インポート完了');
});
...
qs('#btn-finish-stocktake').onclick = async()=>{
  ...
  state.items = await api('items'); renderItems(); alert('棚卸を反映しました'); renderDashboard();
};
