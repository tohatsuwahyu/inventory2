...
function toast(msg, type = 'dark') {
  ...
  // --- TARUH INI DI BAWAH FUNCTION TOAST() DI APP.JS ---
...
// --- 1. FUNCTION BARU: MEMBUAT HEADER 10 KOLOM OTOMATIS ---
function ensureHistoryHeader() {
  ...
  // DEFINISI 10 KOLOM (Agar Sinkron dengan Data)
  const headers = [
    "日時",       // 1. Date
    "ID",        // 2. User ID
    "担当者",     // 3. User Name
    "コード",     // 4. Code
    "品名",       // 5. Item Name
    "数量",       // 6. Qty (AKAN KITA BUAT RATA KANAN)
    "単位",       // 7. Unit
    "種別",       // 8. Type
    "備考",       // 9. Note
    "修正"        // 10. Fix (AKAN KITA BUAT RATA KANAN)
  ];

  // Jika jumlah kolom belum 10, reset dan buat ulang
  if (tr.cells.length !== headers.length) {
    tr.innerHTML = "";
    headers.forEach((text) => {
      const th = document.createElement("th");
      th.textContent = text;
      th.className = "bg-light text-secondary fw-semibold border-bottom";
      
      // --- LOGIC PERATAAN (ALIGNMENT) HEADER ---
      if (text === "数量" || text === "修正") {
        th.style.textAlign = "right"; // Paksa Rata Kanan
  ...
  function bindIO() {
    ...
    if (ioCode) {
      ...
      ioCode.addEventListener("input", (e) => {
       ...
       // Ganti bagian if (!v) di dalam event listener input:
if (!v) {
...
btnStart.addEventListener("click", async () => {
  try {
    ...
    IO_SCANNER = await startBackCameraScan("io-scan-area", async (text) => {
      ...
      // 2. Jika scan berhasil, jangan matikan kamera (kecuali Anda mau)
      // Cukup isi datanya ke form
      if (parsed.kind === "item" || parsed.kind === "lot") {
        const code = parsed.code;
        $("#io-code").value = code;
        await findItemIntoIO(code);
        
        // Update riwayat di preview secara otomatis jika modal sedang terbuka
        if (typeof loadItemHistory === "function") {
          loadItemHistory(code);
        }
      }
    });
  } catch (e) {
    ...
    $("#form-io")?.addEventListener("submit", async (e) => {
      ...
      try {
        const r = await api("log", { 
          method: "POST", 
          body: { userId: who.id, userName: who.name || "", code, qty, unit, type } 
        });

        if (!r?.ok) {
          if (cachedItem) {
            cachedItem.stock = oldStock; // Rollback
            renderItems();
            renderDashboard();
            $("#io-stock").value = oldStock;
          }
          throw new Error(r?.error || "サーバー保存失敗"); 
        }
        ...
        toast("通信エラー: " + err.message, "danger"); 
      } finally {
...
async function findItemIntoIO(codeRaw) {
  ...
  // 2. Kosongkan dulu layar dengan aman (Cek dulu elemennya ada atau tidak)
  ...
  // 4. Jika tidak ada di memori, tanya ke server (Database)
  if (!item) {
    try {
  ...
  // 5. JIKA ITEM KETEMU, TAMPILKAN KE LAYAR
  if (item) {
    ...
    // Beri peringatan jika barang benar-benar tidak terdaftar
    if (code !== "") {
       toast("商品が見つかりません: " + code, "warning");
    }
  }
}
  /* -------------------- Stocktake (棚卸) -------------------- */
  ...
  /* =========================================================
     PERBAIKAN 1: renderShelfTable (Bersih dari Teks Error)
     ========================================================= */
  function renderShelfTable() {
      ...
      <tr data-code="${escapeAttr(r.code)}">
        ...
        <td class="text-end" style="width:100px;">
          <input type="tel" inputmode="numeric" pattern="[0-9]*" 
                 class="form-control form-control-sm st-qty text-end fw-bold" 
    ...
    tbody.oninput = (e) => {
      ...
      if (rec) {
        ...
        // Update tampilan selisih langsung (tanpa render ulang semua)
        const diffCell = tr.cells[3].querySelector("span");
        if(diffCell) {
            diffCell.textContent = (rec.diff > 0 ? '+' : '') + fmt(rec.diff);
            diffCell.className = rec.diff !== 0 ? 'text-danger fw-bold' : 'text-success';
        }
    ...
    tbody.onclick = (e) => {
      ...
      if (btn) {
        const tr = btn.closest("tr");
        const code = tr.getAttribute("data-code");
        if(confirm("リストから削除しますか？")) { // Hapus dari list?
            ST.rows.delete(code);
            renderShelfTable();
        }
      }
...
function bindShelf() {
  ...
  if (btnCommit) {
    btnCommit.addEventListener("click", async (e) => {
      ...
      if (!confirm(`棚卸データ ${rows.length} 件を確定し、在庫を更新しますか？`)) return;
...
// 1. Boot Sidebar & Router saat halaman siap
document.addEventListener('DOMContentLoaded', () => {
...
// Menggunakan Timeout kecil untuk memastikan elemen sudah dirender
setTimeout(() => {
  const btnUniv = document.getElementById('btn-universal-scan');
  
  if (btnUniv) {
    // Clone node untuk membersihkan listener lama agar tidak double/error
    const newBtn = btnUniv.cloneNode(true);
    if(btnUniv.parentNode) {
       btnUniv.parentNode.replaceChild(newBtn, btnUniv);
    }
    ...
    newBtn.addEventListener('click', (e) => {
      ...
      // Mencari section yang memiliki class 'active' ATAU tidak memiliki 'd-none'
      const activeSection = document.querySelector('main section.active') || 
                            document.querySelector('main section:not(.d-none)');
      const viewId = activeSection ? activeSection.id : '';

      if (viewId === 'view-io') {
        // Jika sedang di menu In/Out -> Trigger tombol Scan In/Out
        const btn = document.getElementById('btn-io-scan');
        if(btn) btn.click();
      } 
      else if (viewId === 'view-shelf' || viewId === 'view-shelf-list') {
        // Jika sedang di menu Stock Opname -> Trigger tombol Scan Opname
        const btn = document.getElementById('btn-start-scan');
        if(btn) btn.click();
      } 
      else {
        // Jika di Dashboard/Lainnya -> Pindah ke In/Out dulu, baru Scan
        // Cari tombol navigasi ke IO (bisa di sidebar atau mobile nav)
        const link = document.querySelector('a[data-view="view-io"]') || 
                     document.querySelector('a[onclick*="view-io"]');
        
        if (link) {
          link.click(); // Pindah menu
          
          // Tunggu animasi sebentar (300ms) lalu klik scan otomatis
          setTimeout(() => {
            const btn = document.getElementById('btn-io-scan');
            if(btn) btn.click();
          }, 300);
